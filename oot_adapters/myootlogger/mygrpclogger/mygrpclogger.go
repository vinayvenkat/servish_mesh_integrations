...
//go:generate $GOPATH/src/istio.io/istio/bin/mixer_codegen.sh -a mixer/adapter/mygrpclogger/config/config.proto -x "-s=false -n mygrpclogger -t logentry"
...
import (
    ...
    "github.com/vinayvenkat/myootlogger/mygrpclogger/config"
...
func (s *MyGrpcAdapter) HandleLogEntry(ctx context.Context, in *logentry.HandleLogEntryRequest) (*v1beta1.ReportResult, error) {
	...
	b.WriteString(fmt.Sprintf("HandleLogEntry invoked with:\n  Adapter config: %s\n  Instances: %s\n",
        cfg.String(), instances(in.Instances)))
